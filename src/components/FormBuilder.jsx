...
export function FormBuilder() {
  ...
  const handleMoveField = React.useCallback((fieldId, direction) => {
    if (!fieldId) {
      console.error('No field ID provided for move');
      return;
    }
    ...
    setFormFields(prevFields => {
      ...
      if (direction === 'up' && currentIndex > 0) {
        return arrayMove(prevFields, currentIndex, currentIndex - 1);
      } else if (direction === 'down' && currentIndex < prevFields.length - 1) {
        return arrayMove(prevFields, currentIndex, currentIndex + 1);
      }
